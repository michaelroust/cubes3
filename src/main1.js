...
//   return new Promise(resolve => setTimeout(resolve, 10)); // Simulate 10ms delay
// }

// Asynchronously update the position and rotation of each cube
async function updateCubePositions() {
  await Promise.all(cubes.map(async ({ cube, body }) => {
    const position = body.translation();
    const rotation = body.rotation();

    // Simulate an async operation (can be a network request or some calculation)
...
async function animate() {
  ...
  // Update cube positions asynchronously
  await updateCubePositions();
